...
## Build-time rule
A transaction type that depends on a non-enabled amendment fails on mainnet even though current servers *support* it. MPT, Credential and Oracle builders check live status. Batch is security-retired and `build-batch` is unregistered. For anything else, run `amendment NAME` before building.

## Status snapshot (verify live — this ages)
Checked live 2026-06-10 via `amendment NAME` against mainnet: **enabled** — `MPTokensV1`, `AMMClawback`, `Credentials`, `PriceOracle`, `Clawback`, `TokenEscrow`, `PermissionedDEX`. **Supported but not enabled** — `Batch`, `PermissionDelegation`, `XChainBridge`, `DynamicMPT`, `LendingProtocol`, `SingleAssetVault`. This is protocol status, not Hermes capability; Batch is security-retired/unregistered and the others require explicit tool support.
