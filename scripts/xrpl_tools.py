...
# --- TOOL 24: EVM Sidechain Balance ---
def tool_evm_balance(address: str, network: str = "mainnet"):
    """Query XRP balance on XRPL EVM sidechain via Ethereum JSON-RPC."""
    import httpx
    rpc_urls = {"mainnet": "https://rpc.xrplevm.org", "testnet": "https://rpc.testnet.xrplevm.org"}
    url = rpc_urls.get(network, rpc_urls["mainnet"])
    payload = {"jsonrpc": "2.0", "method": "eth_getBalance", "params": [address, "latest"], "id": 1}
    try:
        ...
        print(f"Error querying EVM sidechain: {e}")
        return
    print(f"Address: {address}")
    print(f"Balance: {xrp:.6f} XRP ({network})")
    print(f"RPC: {url}")

# --- TOOL 25: EVM Contract Deploy ---
def tool_evm_contract(frm: str, bytecode: str, abi: str = None, value: str = "0", gas: str = "200000"):
    """Build an Ethereum contract deployment transaction JSON."""
    tx = {
        "from": frm,
        "data": bytecode if bytecode.startswith("0x") else "0x" + bytecode,
        "value": hex(int(value)),
    ...
    if abi:
        try:
            tx["abi"] = json.loads(abi)
        except Exception:
            tx["abi"] = abi
    print("# EVM Contract Deployment TX — submit to https://rpc.xrplevm.org")
    json_out(tx)

# --- TOOL 26: EVM Bridge Status ---
def tool_evm_bridge(network: str = "mainnet"):
    """Check XRPL EVM sidechain bridge connectivity."""
    import httpx
    rpc_urls = {"mainnet": "https://rpc.xrplevm.org", "testnet": "https://rpc.testnet.xrplevm.org"}
    url = rpc_urls.get(network, rpc_urls["mainnet"])
    payload = {"jsonrpc": "2.0", "method": "eth_blockNumber", "params": [], "id": 1}
    ...
    print(f"RPC: {url}")
    print(f"Chain ID: 1440000")
    print(f"Explorer: https://explorer.xrplevm.org")

# --- HOOK Bit Definitions ---
HOOK_BITS = {
    "onAccountDelete": 1, "onLedgerClose": 2, "onTransaction": 4,
    "onStaked": 8, "onEmit": 16,
    "onURITokenCreateSellOffer": 256, "onURITokenCreateBuyOffer": 512,
    "onURITokenAcceptSellOffer": 1024, "onURITokenAcceptBuyOffer": 2048,
    "onURITokenCancelSellOffer": 4096, "onURITokenCancelBuyOffer": 8192,
}

# --- TOOL 27: Hooks Bitmask ---
def tool_hooks_bitmask(*hook_names: str):
    """Calculate HookOn bitmask from hook event names."""
    mask = 0
    for name in hook_names:
        bit = HOOK_BITS.get(name)
        if bit is not None:
...
# --- TOOL 28: Hooks Account Info ---
def tool_hooks_info(address: str):
    """Query installed hooks on a Xahau account."""
    import httpx
    payload = {"method": "account_objects", "params": [{"account": address, "ledger_index": "validated", "type": "hook", "limit": 20}]}
    try:
        resp = httpx.post("https://xahau.network", json=payload, timeout=15)
        ...
        hooks = data.get("result", {}).get("account_objects", [])
        if not hooks:
            print(f"No hooks installed on {address}")
            return
        print(f"Hooks on {address}: {len(hooks)} installed")
        for i, h in enumerate(hooks, 1):
            h_hash = h.get("HookHash", "?")
            ns = h.get("HookNamespace", "")
            params = h.get("HookParameters", [])
            print(f"  {i}. Hash: {h_hash[:32]}...")
            if ns: print(f"     Namespace: {ns[:20]}...")
            for p in params[:3]:
                pname = p.get("HookParameterName", "?")
                print(f"     Param: {pname}")
...
# --- TOOL 29: Flare FTSO Price Feeds ---
def tool_flare_price(*symbols: str):
    """Fetch live price data via CoinGecko (same data Flare FTSO sources)."""
    import httpx
    coin_ids = {"XRP": "ripple", "BTC": "bitcoin", "ETH": "ethereum", "ADA": "cardano",
                "SOL": "solana", "DOGE": "dogecoin", "DOT": "polkadot", "AVAX": "avalanche-2",
                "LINK": "chainlink", "MATIC": "matic-network", "FLR": "flare-networks"}
    ids = []
    symbol_map = {}
    for sym in symbols:
        s = sym.upper()
        cid = coin_ids.get(s, s.lower())
        ids.append(cid)
        symbol_map[cid] = s
    ...
    try:
        ...
        for cid in ids:
            ...
            sym = symbol_map[cid]
            if price:
                print(f"{sym}: ${price:.6f}")
            else:
                print(f"{sym}: not found")
        print(f"\nData sourced from CoinGecko (same feeds used by Flare FTSO oracles)")
    except Exception as e:
